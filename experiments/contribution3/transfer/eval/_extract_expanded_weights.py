"""Reproduce the best expanded-features DE seed and dump weights.

Runs a single DE seed (seed 4042 = best from offline_sim_expanded.py)
and saves the full 38-element weight vector to JSON.
"""
...
def main():
    t0 = time.time()
    targets = load_data_expanded()
    total = len(targets)
    nf = targets[0][2].shape[1]
    print(f"Targets: {total}, features: {nf}")

    # Run selected seeds — seed 4042 was the best (34/74) in offline_sim_expanded.py
    # Include nearby seeds and a few extras for validation
    seeds_to_try = [4042, 42, 1042, 3042, 5042, 7042, 9042, 11042, 14042]
    bounds = [(-3, 5)] * nf
    ...
    for seed_val in seeds_to_try:
        ...
        result = differential_evolution(
            ...
            seed=seed_val, workers=1, polish=True,
        )
        w = np.array(result.x)
        h = eval_weights(targets, w)
        marker = " *** NEW BEST" if h > best_h else ""
        print(f"  seed {seed_val:5d}: {h}/{total} [{time.time()-t0:.0f}s]{marker}")
        if h > best_h:
            best_h = h
            best_w = w.copy()

    _, missed = eval_detail(targets, best_w)
    print(f"\nBEST: {best_h}/{total}")
