...
Each candidate arrives as **one self-contained representative record**. The `performance_metrics` section is the single most informative evaluation selected upstream. Each record has seven sections:
...
4. **Cluster surviving candidates by study family** and compare siblings on the record: cleaner PRS-only metrics, endpoint definition, training scale, variant coverage, evaluation breadth, covariate cleanliness. Do not rank method labels, publication recency, ancestry breadth, or large evaluation size by themselves — require the records to show why that signal should matter in the particular comparison.
