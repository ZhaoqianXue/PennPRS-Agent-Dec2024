...
Each candidate is one self-contained representative record (the most informative evaluation per candidate is selected upstream) plus, when a European-only representative would otherwise hide multiple non-European or multi-ancestry validation records, a compact validation-ancestry profile. Compare candidates against each other; there is no record-selection step to perform here.

The numbered sections below correspond one-to-one to the eight sections of the single-record PGS schema, so a finding can always be traced to the schema field it came from. The unnumbered **Cross-cutting appraisal principles** layer holds the reasoning that spans fields or candidates and therefore belongs to no single field.
...
- [3. evaluation_ancestry_profile](#3-evaluation_ancestry_profile)
- [4. source_of_variant_associations_gwas](#4-source_of_variant_associations_gwas)
- [5. score_development_training](#5-score_development_training)
- [6. development_method](#6-development_method)
- [7. variants](#7-variants)
- [8. pgs_source — publication context](#8-pgs_source)

---

## Cross-cutting appraisal principles

Empirical patterns that span fields or candidates and therefore map to no single schema field (sections 1–8). The weighing posture and the rough field-importance order live in the skill's procedure (SKILL.md); this layer holds the field- and candidate-spanning knowledge that the procedure draws on.
...
1. **Methodological properties** (§6 Principle A / Principle B) — the strongest structural signal.
...
- Evaluation ancestry (`evaluation_sample.ancestry`) is often more informative than the broad GWAS-ancestry label (§4). PGS portability is governed by how well the **validation ancestry matches the target population**, not by a European default. Multi-ancestry evaluation — especially from a disease-focused multi-cohort study — is the strongest portability evidence. Single-ancestry evaluation is narrower evidence of broad-cohort performance: a high headline metric demonstrated in **only one** ancestry (European-only **or** non-European-only) is a narrower guarantee of broad performance than a modestly-lower metric demonstrated across diverse ancestries. Do **not** treat non-European-only evaluation as a blanket weakness — a validation in any single ancestry is relevant, robust evidence for deployments that include that ancestry, and for a broad/diverse target a single non-European validation is no weaker per se than a single European one. When deployment ancestry is genuinely unspecified, ancestry lowers confidence rather than supporting over-ranking; do not let a single-cohort headline metric override demonstrated cross-ancestry robustness.
...
## 3. evaluation_ancestry_profile

Fields: `evaluation_ancestry_profile` is either `null` or an object with `{record_count, representative_performance_id, selected_validation_ancestry, catalog_eval_ancestry_distribution, observed_validation_ancestries[], additional_validation_records[]}` — compact context showing whether a European-only representative record is only one validation slice of a broader record set.

- Use this section to audit portability and winner's-curse risk in large candidate pools. A high headline metric from the representative record is less convincing when the candidate has broader validation records showing weaker or narrower evidence in other ancestry contexts.
- `observed_validation_ancestries[]` and `catalog_eval_ancestry_distribution` summarize validation breadth. Multi-ancestry or non-European validation is supportive for broad external validation only when the phenotype and metric evidence are coherent; breadth without endpoint fidelity or usable metrics is weak.
- `additional_validation_records[]` are compact summaries of non-representative diverse/multi-ancestry validation records. They are not an instruction to average metrics across records. Read them as corroboration or contradiction of the representative record: do the same endpoint, basic covariates, and meaningful effect-size/performance signals persist outside the selected representative validation context?
- If `additional_validation_records[]` shows consistent endpoint-faithful signal across several ancestry contexts, that strengthens transportability even when the selected representative record is European. If it shows much weaker signal or proxy endpoints outside the selected context, discount a single high headline metric.
- Do not reward ancestry breadth mechanically. A single-ancestry candidate can still be strongest when it has cleaner endpoint fidelity, stronger PRS-comparable metrics or effect sizes, and better study design than a diverse but weak or proxy-profile candidate.

---

## 4. source_of_variant_associations_gwas
...
- Framework models underperform disease-focused models systematically — they generalize less well to independent external cohorts. Identify candidates coming from pan-trait, pan-phenome, or portability-style high-throughput frameworks: the title signatures are listed in §8, and the development profile is characteristic — e.g., L1-penalized sparse pan-trait UKB scores typically have low variant counts, UKB as the sole discovery/development cohort, and standardized covariates, and generalize poorly despite complete metadata.
...
- Variant count is most informative for comparing candidates within the same method family. Cross-method variant-count comparison (e.g., genome-wide shrinkage vs GWAS-hits) just reflects the method distinction from §6 and is not an independent signal.
