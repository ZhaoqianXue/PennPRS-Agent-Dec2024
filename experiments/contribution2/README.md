...
| `disease_selection/runs/selected_diseases_contribution2_current_union__76disease.csv` | Canonicalized current-method union; the numeric suffix tracks the current disease count and is directly usable by recommendation ground-truth generation without manual `Target_TopK` annotation |
...
| `disease_selection/runs/intermediate/selected_diseases_contribution2_current_union_details__76disease.csv` | Audit/detail view for canonical merge decisions and source coverage |
...
# Current canonical union (currently 76 diseases)
python experiments/contribution2/recommendation/configs/generate_evaluated_pgs_list.py \
  --union-csv experiments/contribution2/disease_selection/runs/selected_diseases_contribution2_current_union__76disease.csv
```
...
- plus one extra Markdown report comparing `With Domain Knowledge` vs the matching archived `without-domain-gpt-5.2-t10__30disease` or current-union `without-domain-gpt-5.2-t10__76disease` run
...
# Current canonical union (currently 76 diseases; comparison defaults to the matching without-domain run)
python experiments/contribution2/recommendation/scripts/run_experiment_with_domain.py \
  --union-csv experiments/contribution2/disease_selection/runs/selected_diseases_contribution2_current_union__76disease.csv
...
Per-disease docs are written to dataset-specific filenames:
...
- `recommendation/docs/without_domain_per_disease_comparison__76disease.md`
- `recommendation/docs/with_vs_without_domain_per_disease_comparison__76disease.md`
