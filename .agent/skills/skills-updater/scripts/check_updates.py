...
def get_plugins_dir() -> Path:
    ...
    # Check for local .agent/plugins first
    local_agent_plugins = Path.cwd() / ".agent" / "plugins"
    if local_agent_plugins.exists():
        return local_agent_plugins
    return Path.home() / ".claude" / "plugins"


def get_agent_skills_dir() -> Optional[Path]:
    """Get the Antigravity .agent/skills directory."""
    # Try current directory and parents up to 6 levels to find project root with .agent
    curr = Path.cwd().resolve()
    for _ in range(7):
        if (curr / ".agent").is_dir():
            skills_dir = curr / ".agent" / "skills"
...
def load_installed_plugins() -> Dict:
    ...
    if not plugins_file.exists():
        # Try finding it in any .agent/plugins in parents
        curr = Path.cwd()
        for _ in range(4):
            candidate = curr / ".agent" / "plugins" / "installed_plugins.json"
            if candidate.exists():
...
def load_known_marketplaces() -> Dict:
    ...
    if not marketplaces_file.exists():
        ...
        curr = Path.cwd()
        for _ in range(4):
            candidate = curr / ".agent" / "plugins" / "known_marketplaces.json"
            if candidate.exists():
                marketplaces_file = candidate
                break
            curr = curr.parent
...
def check_all_updates(filter_skill: Optional[str] = None) -> List[SkillInfo]:
    ...
    # 1. Check from installed_plugins.json (standard Claude format)
    ...
    # 2. Check from .agent/skills (Antigravity format)
    ...
    if agent_skills_dir:
        for skill_path in agent_skills_dir.iterdir():
            ...
            # Skip self if nested
            if skill_name == "skills-updater" and skill_path == Path(__file__).parent.parent:
                continue
            ...
            # Check if already processed from plugins.json
            if any(r.name == skill_name for r in results):
                continue

            # For local skills, we try to detect version from SKILL.md frontmatter
            ...
            # Try to find git info if it's a repo
            git_commit_sha = None
            git_dir = skill_path / ".git"
            if git_dir.exists():
                try:
                    res = subprocess.run(["git", "rev-parse", "HEAD"], cwd=skill_path, capture_output=True, text=True)
                    if res.returncode == 0:
                        git_commit_sha = res.stdout.strip()
                except:
                    pass

            # Mock marketplace for display if unknown
            results.append(SkillInfo(
                name=skill_name,
                marketplace="local-agent",
                local_version=local_version,
                remote_version=None,
                status=UpdateStatus.UP_TO_DATE, # Default for local manual skills
