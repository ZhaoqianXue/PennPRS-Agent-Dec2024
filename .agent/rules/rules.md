...
**IMPORTANT**: The `pennprs-agent`, `pgscatalog`, and `TissueLab` folders are **READ-ONLY** and serve **ONLY** as references.

### Rules:
- **DO NOT** modify any files within the `pennprs-agent`, `pgscatalog`, or `TissueLab` folders
- **DO NOT** use any code from these folders directly in the project
- **ONLY** read and reference files in these folders to understand PennPRS API usage (`pennprs-agent`), PGS Catalog tools and data structures (`pgscatalog`), or agentic medical imaging workflows (`TissueLab`)
...
- **TissueLab**: A co-evolving agentic AI system for medical imaging analysis, specifically used for learning single agent tool calling patterns and building robust agentic ecosystems.
