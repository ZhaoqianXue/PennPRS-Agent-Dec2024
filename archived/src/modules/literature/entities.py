...
class PaperMetadata(BaseModel):
    ...
    The abstract is used for classification (Classifier Agent).
    The full_text is used for detailed extraction (Extractor Agents).
    """
    pmid: str = Field(..., description="PubMed ID")
    title: str = Field(..., description="Paper title")
    abstract: str = Field(default="", description="Paper abstract (used for classification)")
    full_text: Optional[str] = Field(default=None, description="Full text content (used for extraction)")
...
class PRSModelExtraction(BaseModel):
    ...
    evidence_html: Optional[str] = Field(default=None, description="HTML snippet with highlighted evidence")
...
class HeritabilityExtraction(BaseModel):
    ...
    Schema aligned with industry standards (Neale Lab, LD Hub, GWAS Atlas).
    Includes fields for statistical significance and QC metrics.
    """
    # Identifiers
    ...
    scale: str = Field(default="not_specified", description="Scale: liability, observed, or not_specified")
    p_value: Optional[float] = Field(default=None, ge=0.0, le=1.0, description="P-value for h² > 0")
    z_score: Optional[float] = Field(default=None, description="Z-statistics for h²")
    
    # Method info
    method: Optional[HeritabilityMethod] = Field(default=None)
    method_detail: Optional[str] = Field(default=None)
    intercept: Optional[float] = Field(default=None, description="LDSC intercept")
    lambda_gc: Optional[float] = Field(default=None, description="Genomic inflation factor")
    
    # Population info
    sample_size: Optional[int] = Field(default=None, ge=1)
    ancestry: Optional[str] = Field(default=None)
    prevalence: Optional[float] = Field(default=None, ge=0.0, le=1.0, description="Population prevalence")
