...
class PubMedClient:
    ...
    def fetch_papers(self, pmids: List[str]) -> List[PaperMetadata]:
        ...
        # Check for truncated abstracts and try to fix them
        papers = self._fix_truncated_abstracts(papers)
        
        return papers
    
    def _fix_truncated_abstracts(self, papers: List[PaperMetadata]) -> List[PaperMetadata]:
        """
        Fix papers with truncated abstracts by fetching from PubMed website.
        
        Some papers have truncated abstracts in the E-utilities API but complete
        abstracts on the PubMed website. This method detects and fixes those cases.
        """
        MIN_ABSTRACT_LENGTH = 500  # Abstracts shorter than this may be truncated
        
        for paper in papers:
            if paper.abstract and len(paper.abstract) < MIN_ABSTRACT_LENGTH:
                # Try to fetch complete abstract from PubMed website
                complete_abstract = self._fetch_abstract_from_web(paper.pmid)
                if complete_abstract and len(complete_abstract) > len(paper.abstract):
                    logger.info(
                        f"Fixed truncated abstract for PMID:{paper.pmid}: "
                        f"{len(paper.abstract)} -> {len(complete_abstract)} chars"
                    )
                    paper.abstract = complete_abstract
        
        return papers
    
    def _fetch_abstract_from_web(self, pmid: str) -> Optional[str]:
        """
        Fetch complete abstract from PubMed website as fallback.
        
        This is used when the E-utilities API returns truncated abstracts.
        """
        import re
        ...
        try:
            ...
            # Extract abstract from HTML using regex
            # Look for the abstract-content div
            pattern = r'<div class="abstract-content[^"]*"[^>]*>(.*?)</div>'
            ...
            if match:
                ...
                if len(abstract_text) > 100:  # Sanity check
                    return abstract_text
            
            return None
            
        except Exception as e:
            logger.warning(f"Failed to fetch abstract from web for PMID:{pmid}: {e}")
            return None
