...
def create_classifier_chain():
    ...
    config = get_config("agentic_classifier")
...
- reasoning: string explaining your decision""")
    ...
    is_strict = getattr(config, 'strict', False)
    
    if is_strict:
        # Strict mode: Force Pydantic output
        # method="json_schema" + strict=True guarantees schema adherence
...
async def classify_with_llm(state: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        ...
        # If strict mode, response is already StudyClassification object
        # If legacy mode, response is AIMessage with string content
        
        if isinstance(response, StudyClassification):
            ...
            classification = StudyClassification(
                study_id=result_dict.get("study_id", study_data.study_id),
                trait_type=result_dict.get("trait_type", "Continuous"),
                sample_size=result_dict.get("sample_size", 0),
                n_cases=result_dict.get("n_cases"),
                n_controls=result_dict.get("n_controls"),
                neff=result_dict.get("neff"),
                ancestry=result_dict.get("ancestry", "EUR"),
