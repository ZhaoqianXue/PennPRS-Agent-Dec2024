...
- `PRS-CS` and `PRS-CS-auto` were not consistently top-tier in this benchmark, but they were also not part of the clearly low-ranked group. Treat them as neutral modern shrinkage methods rather than penalized methods.
...
- mild positive prior: `LDpred2`, `AnnoPred`
- neutral modern middle cluster: `PRS-CS`, `PRS-CS-auto`, `LDpred2-auto`, `LDpred-funct`, `SCT`, `lassosum2`, `SBayesR`, `lassosum`, `DBSLMM`
- clear low prior: `LDpred2-inf`, `LDpred`, `C+T`
...
- Do not treat `PRS-CS` or `PRS-CS-auto` as method liabilities merely because they were not top-tier in the paper; in this knowledge base they should function as neutral modern shrinkage methods unless a clearer negative signal is present.
- If a candidate wins mainly because it is `LDpred2` or `AnnoPred`, keep that advantage very mild; do not let it override materially cleaner endpoint or study-design evidence from another method, including a coherent `PRS-CS` study family.
...
- When comparing two genome-wide shrinkage families such as `LDpred2` versus `PRS-CS`, the literature-derived prior should usually be too weak to decide the match on its own.
